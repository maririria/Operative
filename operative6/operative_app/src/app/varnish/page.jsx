...
export default function Varnish() {
  const [VarnishType, setVarnishType] = useState("matte");
  ...
  const fetchJobs = async () => {
    const currentProcessId = processIds[VarnishType];
    const { data, error } = await supabase
      .from("job_processes")
      .select("*")
          ...
          <div className="flex gap-2">
            {["matte", "shine"].map((type) => (
              <button
                ...
                onClick={() => setVarnishType(type)}
              >
                {type.charAt(0).toUpperCase() + type.slice(1)} Varnish
              </button>
            ))}
          </div>
        </div>
